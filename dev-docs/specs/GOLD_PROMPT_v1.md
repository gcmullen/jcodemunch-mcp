# Gold Annotation Prompt v1.4 — atomic operation (batched anonymized-source-read + expanded audit + method_dispatch forcing example)

**Status:** DRAFT v1.4
**Date:** 2026-05-16
**Changelog from v1.3 (P4.0a F1):** Added an explicit COUNTER-EXAMPLE to the `method_dispatch` schema-clarifications bullet, calling out the D2 dispute pattern where annotators emitted compound 2-word `name` values like `"add command"` or `"configure -option"` instead of the single second-word identifier (`"add"`, `"configure"`). 101 D2 verdicts (~11% of arbiter mass) traced to this naming drift. Convention §5.3 already mandates the single-word form; the forcing example reinforces it prompt-side with two worked counter-examples (Tk menu `$mb add command ...`, snit `$obj configure -option ...`). No convention bump (still v1.3); no compliance_audit shape change. Annotations produced under v1.3 prompt remain valid.
