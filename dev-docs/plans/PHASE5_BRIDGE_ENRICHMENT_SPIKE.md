...
3. **Side-table column strategy (REVISED per OMC critic 2026-05-17).** Earlier draft said "mirror the existing `unresolved_dispatches` pattern in the side-table." That was wrong: `unresolved_dispatches` lives on `Symbol` DIRECTLY (`symbols.py:33`), NOT in the side-table. The actual side-table (`jcm_tcl_extensions`) has 3 columns: `parent_classes` (typed), `package_requires` (typed), `extras_json` (escape valve, comment-tagged as "Phase-2 prototype" at `sqlite_store.py:175`).
   ...
   **As shipped (P5.1):** the migration lives inside `_initialize_jcm_tcl_extensions` itself rather than as a separate gated block. `CREATE TABLE IF NOT EXISTS` declares the v2 6-column schema; then a `PRAGMA table_info(jcm_tcl_extensions)` check drives idempotent `ALTER TABLE … ADD COLUMN` for `callees_json` and `args_json` whenever they're absent. The function is called on every save_index / incremental_save / migration-path connect, so v1-era tables get promoted in-place the first time their owning DB is touched by P5.1+ code. The load path treats NULL `callees_json` / `args_json` as `[]` (the R1 contract). Tests in `TestV1IndexBackwardCompat` lock both the NULL-tolerance and the ALTER-TABLE migration.
...
2. **5.1 (schema) — SHIPPED:** added `Symbol.callees` + `Symbol.args` fields + `jcm_tcl_extensions` side-table typed columns (`callees_json`, `args_json` per §4) + serialization roundtrip + backward-compat read tests for v1-era indexes + the `JCM_TCL_INDEX_VERSION 1 → 2` bump. Migration via idempotent `ALTER TABLE … ADD COLUMN` inside `_initialize_jcm_tcl_extensions` (PRAGMA-gated; runs on every save / migration-path connect). Tests in `TestCalleesAndArgsRoundTrip` and `TestV1IndexBackwardCompat`. `TestVersionBumpPath` was refactored to use `JCM_TCL_INDEX_VERSION + 1` so it survives every future bump.
3. **5.2 (walker rules, NO class-DSL handling here):** wire `parser/tcl/opcode_walker.tcl` + `parser/tcl/bridge_postpasses.tcl` to emit `callees` per-call-site records (parallel to `call_references`, not replacing it). Specifically:
   - §7.1 Tier 1/2/3/5 filter pass (new pass in `bridge_postpasses.tcl`) — **SHIPPED in 5.2.1**.
   - §5.3 method_dispatch emission on `$obj method` patterns — 5.2.6.
   - §6.9/§6.12 callback emission from script-accepting sites — 5.2.7.
   - §5.10/§7.5 2-word kept-ensemble emission — 5.2.5.
   - ~~§5.2 qualified-name preservation (`mc` → `msgcat::mc`)~~ — **NOT NEEDED (5.2.2 investigation 2026-05-17).** The bridge already preserves qualified names verbatim per convention §5.2; the canary probe and `tests/test_tcl_parser.py::TestQualifiedNamePreservation` lock this. The verdict §3 / §6 bug #4 "mc 51" was a misclassification: gold annotators recorded `mc` (verbatim source form), not `msgcat::mc`. The 51-miss count is multiset semantics — gold has one entry per call site, the bridge's `call_references` dedupes by name. That gap closes when 5.2.6 populates `callees` with one record per call site.
   - §5.5 visibility-prefix recognition (`public`/`private`/`protected` not callee word) — 5.2.3.
   - §6.8 operator exclusion (no `==` / `&&` / `>` / `ne` / `eq` from `expr {...}` operands) — 5.2.4.
...
4. **STAGED GATE between 5.2 and 5.2a (per OMC critic) — MET:** after 5.2 lands, run the §7.5 disposition matrix's strict-diff check. **5.2 must achieve recall ≥ 0.65 (~0.39 + bug-1 + bug-2 + bug-3 deltas) on the v2 diff before 5.2a starts.** If 5.2 alone is below 0.65, hold for triage — the walker rules might not be capturing what we expected; investigating before adding DSL handling is cheaper than debugging both layers at once. **2026-05-17 outcome:** initial 5.2.7 closeout hit 0.2609 (HOLD); triage diagnosed missing static + qualified emission at 8 unpaired `_add_call_to_parent` sites; 5.2.X wired them; re-measurement reached **0.7182** (gate met). See `dev-docs/verdicts/BRIDGE_VS_GOLD_VALIDATION_v2.md`. The triage discipline worked exactly as the critic designed — fast diagnosis + targeted fix rather than piling DSL handling on a broken foundation.
